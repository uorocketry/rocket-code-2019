...
SBG_ECOM_CMD_INFO = 4
SBG_ECOM_SYNC_1 = 0xFF
SBG_ECOM_SYNC_2 = 0x5A
SBG_ECOM_MAX_BUFFER_SIZE = 4096
SBG_NO_ERROR = 0
SBG_ECOM_ETX = 0x33  # end of frame byte
crc_16_lookup_table = np.array([0x0000, 0x1189, 0x2312, 0x329B, 0x4624, 0x57AD, 0x6536,
                                ...
                                0x3DE3, 0x2C6A, 0x1EF1, 0x0F78], dtype='uint16')
...
def sbg_send(serial, message_class, message, size, payload=None):
    to_send = np.array([],dtype='uint8')
    to_send = np.append(to_send, np.uint8(SBG_ECOM_SYNC_1))
    to_send = np.append(to_send, np.uint8(SBG_ECOM_SYNC_2))
    to_send = np.append(to_send, np.uint8(message))
    to_send = np.append(to_send, np.uint8(message_class))
...
def sbg_crc_compute(buffer):
    crc_16 = np.uint16(0)
    for i in range(len(buffer)):
        index = np.bitwise_and(np.bitwise_xor(buffer[i], crc_16), 0xFF)
        crc_16 = np.bitwise_or(crc_16_lookup_table[index], np.right_shift(crc_16, 8))
    return np.uint16(crc_16)
